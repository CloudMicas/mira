...
5. **DeepSeek provider 未接入工厂**：代码在但 `createLLMProvider` 没暴露切换入口
6. **无鉴权与限流**：WS 裸奔，任何人可连；会话存内存 Map，不能水平扩展，刷新即丢对话
7. **指令遵循靠 prompt 约束**：LLM 偶尔漏发/发错指令块，无 schema 校验与重试
8. **角色模型是通用 Hiyori**，并非 Mira 专属立绘；pose 只有 4 种、fx 只有 3 种，演出表现力有限

## 七、大致投入时间

约 **10 小时**（含技术选型、协议设计、前后端联调与 Docker 化）。
